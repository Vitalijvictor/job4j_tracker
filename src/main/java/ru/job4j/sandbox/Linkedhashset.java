 ...
 * которая поддерживает двусвязный список для всех элементов.
 * Когда необходимо поддерживать порядок итераций,
 * используется этот класс. При переборе HashSet порядок непредсказуем,
 * а LinkedHashSet позволяет перебирать элементы в том порядке,
 * в котором они были вставлены.
 * При циклическом переходе по LinkedHashSet с использованием
 * итератора элементы будут возвращены в том порядке,
 * в котором они были вставлены.
 */
